...
public class Parser {
    /**
     * Mot clé permettant de reconnaitre '.PHONY', sous forme d'une expression
     * régulière
     */
    ...
    /**
     * Le modèle permettant de reconnaitre les assignations de variables d'un
     * Bakefile
     */
    private static final Pattern VARIABLE_ASSIGN_PATTERN = Pattern
            .compile("^\\s*(?<key>[a-zA-Z_][a-zA-Z0-9_.-]*)\\s*=\\s*(?<value>.+)", Pattern.MULTILINE);
     ...
     */
    private static final Pattern PHONY_PATTERN = Pattern
            .compile(String.format("%s[ \\t]*:[ \\t]*(?<refs>(?:[a-zA-Z._-]+[ \\t]*)*)", Parser.PHONY_KEYWORD));
     ...
     * @return Le contenu de la chaine de caractères avec les variables remplacées
     *         par leurs valeurs
     */
    private static String replaceVars(String content) {
        ...
        // Comme il est possible d'invoquer des variables dans d'autres variables, il
        // est important de réxécuter le
        // programme de remplacement tant qu'il y a des variables encore non-touchées
        // présentes dans le programme.
    ...
    /**
     * Crée une instance de cette à partir d'un nom de fichier en récupérant son
     * contenu
     * 
    ...
    public static Parser fromFile(String filename) throws FileNotFoundException {
        ...
        // Retire directement tous les commentaires d'un texte de façon à éviter d'être
        // dérangés durant l'exécution
        // du programme
     ...
     * Cette méthode crée un bloc en utilisant la méthode statique de parsing
     * de la classe 'Block'
     * 
     * @see fr.tanchoulet.bakefile.Block
     * @see fr.tanchoulet.bakefile.Block#parse(String, List)
     *
    ...
    public BlockList parse() {
        ...
        while (blockMatcher.find()) {
            ...
            // Étant donné que la formation d'un bloc '.PHONY' à la même syntaxe qu'un bloc
            // général, il est important
            // de retirer les occurrences de '.PHONY' des blocs
            if (block.name.matches(Parser.PHONY_KEYWORD))
                continue;
